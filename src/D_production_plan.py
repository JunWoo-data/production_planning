...
def Line3_production_plan_summary(df, start_date, end_date, production_summary):
    ...
    for k, v in production_summary.items():
        ...
        # print(f"    --> {num_change_over} x 5 mins = {num_change_over * 5} mins downtime")
        summary_text_list.append(f"----> {num_change_over} x 5 mins = {num_change_over * 5} mins downtime")
        
        production_deduction = math.ceil(num_change_over * 5 * uph / 60)
        # print(f"    --> UPH: {uph} -> {num_change_over * 5} mins: {production_deduction} production availability deduction")
        summary_text_list.append(f"----> UPH: {uph} -> {num_change_over * 5} mins: {production_deduction} production availability deduction")
...
def Line2_production_plan_summary(df, start_date, end_date, production_summary):
    ...
    for k, v in production_summary.items():
        if len(v) >= 1:
            ...
            summary_text_list.append(f"---- Between different program: {num_change_over_btw_dfft_program} -> {15 * num_change_over_btw_dfft_program} mins downtime")
            # print(f"---- Between same program: {num_change_over_btw_same_program} -> {5 * num_change_over_btw_same_program} mins downtime")
            summary_text_list.append(f"---- Between same program: {num_change_over_btw_same_program} -> {5 * num_change_over_btw_same_program} mins downtime")
            
            total_changeover_downtime = 15 * num_change_over_btw_dfft_program + 5 * num_change_over_btw_same_program
